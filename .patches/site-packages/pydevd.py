...
class PyDB(object):
    ...
    def __init__(self, set_as_global=True):
        ...
        self.mapping_patches = mapping_patches
    ...
    def keyshot_patch(self, abs_path_real_path_and_base, frame):
        if not hasattr(self, "mapping_patches"):
            print("mapping_patches attribute missing. No patches for remote debugging file mapping applied.")
            return abs_path_real_path_and_base

        if abs_path_real_path_and_base[-1] in self.mapping_patches:
            incorrect_mapping = abs_path_real_path_and_base[-1]
            mapping = self.mapping_patches[incorrect_mapping]

            abs_path_real_path_and_base = list(abs_path_real_path_and_base)
            for i in range(len(abs_path_real_path_and_base)):
                abs_path_real_path_and_base[i] = abs_path_real_path_and_base[i].replace(incorrect_mapping, mapping)
            abs_path_real_path_and_base = tuple(abs_path_real_path_and_base)

            NORM_PATHS_AND_BASE_CONTAINER[frame.f_code.co_filename] = abs_path_real_path_and_base
        return abs_path_real_path_and_base
    ...
    def set_trace_for_frame_and_parents(self, frame, **kwargs):
        ...
        while frame is not None:
            ...
            abs_path_real_path_and_base = self.keyshot_patch(abs_path_real_path_and_base, frame)
            #print(f"abs_path_real_path_and_base: {abs_path_real_path_and_base}")
